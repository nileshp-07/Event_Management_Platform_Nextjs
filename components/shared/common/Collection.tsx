...
const Collection = ({events, emptyMsg, emptyStateSubtext, limit, page, totalPages, collectionType, urlParamName}: CollectionType) => {
  return (
    <>
        {
            events?.length > 0 ? (
                <div className='flex flex-col items-center gap-10'>
                    ...
                    {
                        totalPages! > 1 && (
                            <Pagination urlParamName={urlParamName} page={page} totalPages={Number(totalPages)}/>
                        )
                    }
                </div>
            ) : (
